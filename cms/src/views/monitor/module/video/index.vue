	  ...
	  data () {
    	return {
	        ...
	        sources: [{
          	...
          	src: 'http://hls.open.ys7.com/openlive/asd.m3u8',
	        }],
  		...
  		init () {
  			...
  			fetchList("electricitySubstation_video",param).then( res => {
  				if(res.data && res.data.items) {
  					// alert(res.data.items[0].url)
  					console.log(res.data.items[0].url)
  					this.playerOptions.sources[0].src = res.data.items[0].url
  					// this.playerOptions.sources[0].src = 'http://hls.open.ys7.com/openlive/f0aa5ad188aa49ab9e6d17eadbadfad5.m3u8' || res.data.items[0].url;
  				}
  			})
  		},
